...
export default function GeneralActivityLogClient() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      const statsData = await getGeneralAuditLogStatsAction();
      setStats(statsData);
    } catch (error) {
  ...
  const handleExport = async () => {
    setIsExporting(true);
    try {
      // Add filter to exclude login/logout activities for general activity export
      const exportFilters = {
        ...filters,
        excludeActions: ["LOGIN", "LOGOUT"] // Exclude authentication activities from export
      };
      ...
      a.href = url;
      a.download = `general_activity_log_${format(new Date(), 'yyyy-MM-dd_HH-mm-ss')}.csv`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
      
      showToast.success("General activity log exported successfully");
    } catch (error) {
  ...
  const getActionBadgeVariant = (action: string) => {
    switch (action.toLowerCase()) {
      case "create": case "activate": return "default";
