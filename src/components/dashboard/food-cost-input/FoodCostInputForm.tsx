...
import type { Category, FoodCostEntry, FoodCostDetail, Property, User, Outlet } from "@/types";
import { createFoodCostEntryAction, updateFoodCostEntryAction } from "@/actions/foodCostActions";
  ...
  async function onSubmit(data: FoodCostInputFormValues) {
    ...
    try {
      const itemsToSave = data.items.map(item => {
        const category = foodCategories.find(cat => cat.id.toString() === item.categoryId);
        return {
          id: item.id, 
      ...
      const totalFoodCost = itemsToSave.reduce((sum, item) => sum + item.cost, 0);

      if (existingEntry) {
        // Update existing entry
        await updateFoodCostEntryAction(existingEntry.id, {
          date: selectedDate,
        ...
        showToast.success("Food cost entry updated successfully.");
      } else {
        // Create new entry
        ...
        showToast.success("Food cost entry created successfully.");
      }
